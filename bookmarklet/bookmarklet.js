javascript:(async function() {
    ...
    const lks = Array.from(document.querySelectorAll('img')).filter(m => m.src.toLowerCase().endsWith('.jpg') && !m.src.includes('icons/')).map(m => ({g: m.src.replace(/\.jpg$/i, '.gif'), h: m.closest('a')?.href}));
    if (!lks.length) return;
    const d = document, b = d.createElement('div'), mn = d.createElement('div'), v = d.createElement('canvas'), azc = d.createElement('canvas'), s = d.createElement('input'), l = d.createElement('div'), t = d.createElement('div'), p = d.createElement('div'), x = d.createElement('button');
    ...
    b.style = 'position:fixed;inset:0;background:#fff;z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:center;font:14px sans-serif;overflow:auto;';
    mn.style = 'display:flex;align-items:center;justify-content:center;gap:30px;width:100%;max-height:65vh;';
    v.style = 'max-width:60%;max-height:100%;outline:1px solid #ccc; background:#000;';
    ...
    x.innerText = 'CHIUDI'; x.style = 'position:absolute;top:20px;right:20px;padding:10px;cursor:pointer;';
    ...
    startBtn.innerText = 'SCARICA E ANALIZZA DATI (' + lks.length + ' file)';
    startBtn.style = 'padding:10px 20px; cursor:pointer; font-weight:bold; background:#4CAF50; color:#fff; border:none; border-radius:4px;';
    ...
    function dr(n) {
        ...
        if (data.length > 0) {
            ...
            l.innerText = `Frame: ${parseInt(n) + 1} / ${data.length}`;
            t.innerText = `SOL: ${o.sol} | DATA: ${o.t}\nAZ: ${o.az}° | EL: ${o.el}° | DIST: ${dist.toFixed(2)}m`;
            ...
            l.innerText = 'Pronto'; t.innerText = 'Calibrazione asimmetrica +6/-7m. FOV: 14.4°x14.0°.';
        }
    }
    ...
    s.oninput = () => dr(s.value); mn.append(v, azc); b.append(mn, s, l, t, p, downloadArea, x); d.body.append(b); dr(0);
})();
